...
def main():
    ...
    with DatabaseConnection(config=config.dbconfig) as connection:
        ...
        insert_order = utils.get_insert_order(sql_procedure_path=config.DEPENDENCIES_PROCEDURE, connection=connection)


        tables = {}
        for table in insert_order:
            columns = set(integrated_db.get_columns(table))
            tables[table] = Table(table_name=table, connection=connection, valid_columns=columns)
